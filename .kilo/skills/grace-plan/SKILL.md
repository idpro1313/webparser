...
- critical scenarios that must be checked, including **separate success, failure, and alternate-path scenarios** where behavior diverges
- **per-branch** log or trace anchors: mutually exclusive arms must be distinguishable in output (`rules/grace-logging.md`)
...
### Phase 2: Hold Superposition Before Architecture Collapse
For non-trivial architecture work, do not jump to the first plausible design.

1. Define 3-5 success criteria such as reliability, implementation speed, testability, autonomy readiness, dependency risk, and operational simplicity.
2. Propose 2-3 meaningfully different architecture concepts.
3. Evaluate each concept against the criteria.
4. Choose the concept that best fits the criteria and explain why it is the selected collapse point.

Skip this only when the user already chose the architecture, the task is a narrow targeted change, or existing compatibility constraints leave only one viable option.

### Phase 3: Design Module Architecture
...
### Phase 4: Design Verification Surfaces
...
- identify required log markers or trace evidence for **each** critical branch (success, rejection, early exit, alternate business arm—not only the default path)
