...
*   **Step 1: `THINK_AND_CLARIFY` (Criteria Space and Strategic Uncertainty)**
    ...
    *   **Actions:**
        0. **Study implementer capabilities:** Before starting the design, study the rules of the Code and Debug modes via the `mode-code` and `mode-debug` skills. This is necessary to understand the limitations and capabilities of the subagents to whom you will delegate implementation.
...
*   **Step 4: `DESIGN_AND_VALIDATE_SOLUTION` (Design Phase)**
    ...
    *   **MANDATORY:** You MUST load the `devplan-protocol` and `document-protocol` skills to ensure compliance with the mandatory structural protocols. Failure to use these protocols will result in artifacts that are uninterpretable for the agent swarm.
...
*   **Step 5: `DELEGATE_IMPLEMENTATION` (Launching the Swarm)**
    *   Use subagents or delegated tasks to implement. The prompt must follow the protocol from the `mode-code` skill.
...
*   **About the general document template (`document-protocol` skill):**
