...
- preserve or add stable log markers for **every behavior-changing branch** on the module’s critical path (else arms, early returns, error paths), per `rules/grace-logging.md`
...
Checkpoint: assumptions confirmed / retry budget used N / stop trigger нет
