...
- did it follow an acceptable execution path (**which branches ran**, in production and in tests)?
...
- each **behavior-changing branch** is visible in the trace: **if/else/switch** arms, guard exits, retries/fallbacks, and error paths—not only the happy path; mutually exclusive arms must be **distinguishable** (distinct markers or one marker plus a stable `branch` / `outcome` / `reasonCode` field)
...
- production and tests must target the **same canonical marker strings or fields** so trajectory evidence compares across environments

Prefer stable structured logs or stable key fields over prose-heavy log lines.

Use two logging tiers:
- **Default GRACE markers:** `[ModuleName][functionName][BLOCK_NAME]` plus stable structured fields. This is enough for most governed modules.
- **Strict LDD markers:** `[CLASSIFIER][IMP:1-10][FUNCTION][BLOCK][OPERATION] Description [STATUS]`. Use this for high-risk, high-autonomy, hard-to-reproduce, financial, security, data-integrity, or external-side-effect flows.
