...
Use the Diagnostic Trio when the failure is not obvious:
- **Logs:** compare expected markers, required trace order, error fields, and high-importance LDD entries with the observed evidence. If logs show **that** something failed but not **which branch** was taken, strengthen branch markers per `rules/grace-logging.md` as part of the fix, not only the functional outcome.
- **Code:** inspect the contract, semantic block, side effects, and dependency calls around the first divergent point.
- **Data:** inspect fixtures, persisted state, DB rows, files, or test outputs needed to prove the mismatch.

### Step 4: Fix
Apply the fix WITHIN the semantic block boundaries. Do NOT restructure blocks unless the fix requires it.

For complex, repeated, or subtle bugs, add a compact `BUG_FIX_CONTEXT` note near the modified semantic block explaining why the old approach failed and why the new one preserves the contract.
