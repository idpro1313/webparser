...
- [ ] required log markers or trace anchors still exist and are stable; **important alternate branches** (else/error/early return) are covered in scenarios and markers, not only the default path
...
When evidence is ambiguous or a test failure is being reviewed, use the Diagnostic Trio:
- Logs: expected vs observed markers, trace order, and high-importance LDD evidence.
- Code: contract, implementation, side effects, and first divergent semantic block.
- Data: fixtures, persisted state, DB/files, or command output proving the behavior.
